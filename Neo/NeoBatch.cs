...
{
	private readonly NeoBatcher _batcher;
	private readonly Effect _effect;

	private readonly GraphicsDevice _graphicsDevice;
	private readonly Neo _neo;

	private float _lastScale;
	private Viewport _lastViewport;
	private Matrix _matrix;
	private Vector2 _texCoordBR = new(0, 0);
	private Vector2 _texCoordTL = new(0, 0);

	/// <param name="graphicsDevice">The <see cref="GraphicsDevice" />, which will be used for sprite rendering.</param>
	/// <param name="capacity">
	///     The initial capacity of the internal array holding batch items (the value will be rounded to the
	///     next multiple of 64).
	/// </param>
	...
	public void DrawGlyph(Texture2D atlas, Rectangle destinationRectangle, float left, float bottom, float right,
		float top, Color color)
	{
		...
		_texCoordTL.X = left * (1f / atlas.Width);
		_texCoordTL.Y = ntop * (1f / atlas.Height);
		_texCoordBR.X = right * (1f / atlas.Width);
		_texCoordBR.Y = nbottom * (1f / atlas.Height);
		...
		Vector2 origin, Vector2 scale, SpriteEffects effects, float layerDepth)
	{
		...
		if (sourceRectangle.HasValue)
		{
			...
			_texCoordTL.X = srcRect.X * (1f / texture.Width);
			_texCoordTL.Y = srcRect.Y * (1f / texture.Height);
			_texCoordBR.X = (srcRect.X + srcRect.Width) * (1f / texture.Width);
			_texCoordBR.Y = (srcRect.Y + srcRect.Height) * (1f / texture.Height);
		}
		else
		{
		...
		float rotation, Vector2 origin, SpriteEffects effects)
	{
		...
		else
		{
			...
			origin.X = origin.X * destinationRectangle.Width * (1f / texture.Width);
			origin.Y = origin.Y * destinationRectangle.Height * (1f / texture.Width);
		}
