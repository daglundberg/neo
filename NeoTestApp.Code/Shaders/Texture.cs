...
namespace NeoTestApp.Code
{
	public class Texture
	{
		...
		private Texture2D _flowMap;
		private TextureVertex[] _vertices;
		private Techniques _technique;
		public Texture(string textureName, Vector2 position, Techniques technique) : this(Game1.MyContent.Load<Texture2D>(textureName), position, technique) { }
		...
		public void Draw(GameTime gameTime, Camera camera, Vector3 position, float alpha = 1, float scale = 1, float rotation = 0)
		{
			...
			_effect.Parameters["FlowMap"].SetValue(_flowMap);
